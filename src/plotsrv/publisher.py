...
def _try_publish_artifact_pathlike_compat(
    obj: Any,
    *,
    host: str,
    port: int,
    ...
    """
    Compatibility path for publish_artifact(Path(...)).

    This keeps old tests and old user code behaviour intact while publish_view()
    becomes the preferred general-purpose API.
    """
    if isinstance(obj, (str, bytes, bytearray)):
        return False
...
def publish_artifact(
    ...
    *,
    ...
    """
    Compatibility wrapper for the old artifact-oriented API.

    Prefer publish_view(...) in new code.

    Historical compatibility:
      - Path-like files still route through file-kind coercion
      - DataFrames still route to publish_view(..., kind="table")
      - Plot-like objects still route to publish_view(..., kind="plot")
      - Other objects route through the generic publish_view(...) path
    """
    debug = _debug_enabled()

    if _try_publish_artifact_pathlike_compat(
        obj,
        host=host,
        port=port,
        label=label,
