...
def run_command(args: argparse.Namespace) -> int:
    ...
    spec = RunSpec(
        ...
        watch_max_mb=(
            None if str(args.watch_max_mb).strip().lower() == "off" else float(args.watch_max_mb)
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = sub.add_parser("run", help="run one baseline, publishing, or watched-file scenario")
    run.add_argument(
        "--scenario",
    ...
    run.add_argument("--output", help="New output directory. Defaults under benchmark-results/.")
    run.add_argument("--table", action="append", help="Repeatable table shape, e.g. 5000x12.")
    run.add_argument("--json-items", action="append", type=int, help="Repeatable JSON item count.")
    ...
    run.add_argument("--cpu-s", type=float, help="CPU work per iteration before publishing.")
    ...
    run.add_argument("--max-rss-mb", type=int, help="Kill the complete benchmark process tree above this aggregate RSS.")
    run.add_argument("--watch-csv", help="Watched CSV shape for watch scenarios, e.g. 500000x20.")
    run.add_argument("--watch-materialization", choices=["memory", "file"], default="file")
    run.add_argument("--watch-max-mb", default="16", help="Maximum watched-file preview size, or 'off'.")
    ...
    compare = sub.add_parser("compare", help="compare the run.json summaries from two runs")
